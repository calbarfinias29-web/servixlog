-- ============================================================
-- FIX: safe_update_job_status — 42804 "column status is of type
-- job_status but expression is of type text"
--
-- CAUZĂ: parametrul p_status este declarat `text`, iar
-- `UPDATE jobs SET status = p_status` atribuie direct o valoare
-- `text` unei coloane enum (`job_status`), fără cast explicit.
-- Postgres nu are un cast implicit/de atribuire text -> enum
-- definit de utilizator, deci orice apel eșua cu 42804.
--
-- FIX: cast explicit `p_status::job_status` DOAR la scrierea în
-- coloană, după validarea existentă a valorilor permise
-- ('asteptare', 'asteptare_piese', 'finalizat'). Nicio altă logică
-- nu se schimbă (worked_seconds, overtime_seconds, started_at,
-- is_overtime, completed_at, activity_log rămân identice).
-- ============================================================
