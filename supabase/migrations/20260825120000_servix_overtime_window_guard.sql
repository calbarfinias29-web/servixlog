...
--
-- safe_start_overtime / safe_stop_overtime / servix_can_start_overtime
-- referențiau un tabel `schedules` care nu există în schema proiectului.
-- Schema reală folosește `work_schedule` (20260824115251, rând activ
-- 07:00–18:00, pauză 13:00–14:00) — același tabel folosit de frontend.
--
-- Recrează cele 3 funcții cu tabelul corect, păstrând INTACTE regulile:
...
-- Idempotentă: sigură la rerulare (CREATE OR REPLACE).
-- ============================================================
...
BEGIN
  ...
  -- Contabilitate separată: NU se atinge worked_seconds.
  UPDATE jobs
  SET started_at = now(),
      is_overtime = true
  WHERE id = p_job_id;
...
BEGIN
  ...
  UPDATE jobs
  SET overtime_seconds = overtime_seconds + v_elapsed,  -- DOAR overtime
      started_at = NULL,
      status = 'asteptare',
      is_overtime = false
  WHERE id = p_job_id;
...
-- started_at = now(); worked_seconds NU este atins aici (contabilitate separată).
CREATE OR REPLACE FUNCTION safe_start_overtime(p_job_id uuid, p_employee_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Tabelul real al programului de lucru este work_schedule (nu `schedules`).
  SELECT * INTO v_schedule FROM work_schedule WHERE active = true ORDER BY id LIMIT 1;
  IF FOUND THEN
    v_local := (now() AT TIME ZONE 'Europe/Bucharest')::time;
    IF NOT (
...
-- worked_seconds rămâne neatins (normalul e înghețat după 18:00 / în pauză).
CREATE OR REPLACE FUNCTION safe_stop_overtime(p_job_id uuid, p_employee_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
