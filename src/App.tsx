...
import type { CarUpdateInput, DevicePairingResult, DeviceType, ManagedDevice } from './data/DataAdapter';
...
// Temă Dark pentru panoul de administrare — paletă dedicată Admin, cu
// niveluri de suprafață mai distincte și accente mai vizibile decât
// Employee Dark (nu se ating token-urile Employee/Light).
...
function AdminPanel({ employees, cars, appointments, schedule, rates, themes, vehicleMakes, vehicleModels, workCatalog, onRefresh, onExit, adminTheme, employeeTheme, onChangeAdminTheme, onChangeEmployeeTheme }: { employees: Employee[]; cars: Car[]; appointments: Appointment[]; schedule: Schedule | null; rates: Rates | null; themes: Theme[]; vehicleMakes: CatalogOption[]; vehicleModels: CatalogOption[]; workCatalog: CatalogOption[]; onRefresh: () => Promise<void>; onExit: () => void; adminTheme: 'light' | 'dark'; employeeTheme: 'light' | 'dark'; onChangeAdminTheme: (m: 'light' | 'dark') => void; onChangeEmployeeTheme: (m: 'light' | 'dark') => void }) {
  ...
  const [editCarState, setEditCarState] = useState<Car | null>(null);
  const [showAdd, setShowAdd] = useState(false);
...
function DashboardView({ employees, cars, appointments, rates, schedule, employeeName, onRefresh, onShowCar, onAddCar, onGoToCars, onGoToAppointments, onGoToEmployees, onGoToReports }: {
  ...
  const statusCards: Array<{ key: 'intarziata' | 'piese' | 'finalizata' | 'in_lucru'; label: string; list: Car[]; bg: string; dot: string; Icon: React.ElementType }> = [
    { key: 'intarziata', label: 'ÎN URMĂ CU TERMEN', list: intarziateList, bg: 'color-mix(in srgb, var(--danger) 10%, transparent)', dot: 'var(--danger)', Icon: AlertTriangle },
  ...
  const finRows: Array<{ label: string; n: number; color: string }> = [
    ...
    { label: 'Neîncasate', n: finalizedAll.filter((c: Car) => c.financial_status === 'neincasat').length, color: 'var(--danger)' },
    { label: 'Facturate', n: finalizedAll.filter((c: Car) => c.financial_status === 'facturat').length, color: 'var(--info)' },
    { label: 'Nefacturate', n: finalizedAll.filter((c: Car) => c.financial_status === 'nefacturat').length, color: 'var(--primary)' },
  ];
...
<div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-lg" style={{ background: 'color-mix(in srgb, var(--info) 14%, transparent)', color: 'var(--info)' }}><Clock3 size={18} /></div>
...
<circle cx="70" cy="70" r={donutR} fill="none" stroke="var(--border)" strokeWidth="14" />
...
<text x="70" y="66" textAnchor="middle" fontSize="22" fontWeight="700" fill="var(--text-primary)">{finTotal}</text>
<text x="70" y="84" textAnchor="middle" fontSize="9" fontWeight="600" fill="var(--text-secondary)">TOTAL</text>
</svg>
...
<div className="mt-2 h-1.5 w-full overflow-hidden rounded-full" style={{ background: 'var(--border)' }}><div className="h-full rounded-full" style={{ width: `${Math.round((sec / topMax) * 100)}%`, background: SV.purple }} /></div>
</div>)}
</div>
...
function ReportsView({ cars, employees, rates, schedule, employeeName, onRefresh }: { cars: Car[]; employees: Employee[]; rates: Rates | null; schedule: Schedule | null; employeeName: (id: string | null) => string; onRefresh: () => Promise<void> }) {
  ...
  const kpiCards: Array<{ label: string; count: number; bg: string; dot: string; Icon: React.ElementType }> = [
    ...
    { label: 'Neîncasate', count: neincasate.length, bg: 'color-mix(in srgb, var(--danger) 10%, transparent)', dot: 'var(--danger)', Icon: AlertTriangle },
...
function AddAppointmentModal({ cars, employees, onClose, onSaved }: { cars: Car[]; employees: Employee[]; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  return <Modal title="Adaugă programare" onClose={onClose}><div className="space-y-5 p-6">
    ...
    {error && <div className="rounded-lg border border-[color-mix(in_srgb,var(--danger)_35%,transparent)] bg-[color-mix(in_srgb,var(--danger)_12%,transparent)] px-4 py-3 text-sm font-semibold text-[var(--danger)]">{error}</div>}
...
function EventModesCard({ employees, onRefresh }: { employees: Employee[]; onRefresh: () => Promise<void> }) {
  ...
  return <div className="rounded-[18px] border bg-[var(--surface)] p-6" style={{ borderColor: SV.border }}>
    ...
    {error && <div className="mt-4 rounded-lg border border-[color-mix(in_srgb,var(--danger)_35%,transparent)] bg-[color-mix(in_srgb,var(--danger)_12%,transparent)] px-3 py-2.5 text-xs font-semibold text-[var(--danger)]">{error}</div>}
...
{message && <span className="text-sm font-semibold text-[var(--success)]">{message}</span>}
{saveError && <span className="text-sm font-semibold text-[var(--danger)]">{saveError}</span>}
...
{empError && <div className="mt-4 rounded-lg border border-[color-mix(in_srgb,var(--danger)_35%,transparent)] bg-[color-mix(in_srgb,var(--danger)_12%,transparent)] px-3 py-2.5 text-xs font-semibold text-[var(--danger)]">{empError}</div>}
<div className="mt-5 space-y-4">
...
// EDIT CAR MODAL
// ============================================================
// Modifică o mașină EXISTENTĂ (UPDATE pe aceeași id — NU inserează duplicate).
// Câmpurile formularului reutilizează aceeași structură ca „Adaugă mașină”
// (componenta Field + selecturile existente). Salvează PRIN DataAdapter
// (dataAdapter.updateCar), astfel încât să funcționeze identic pe Web/Supabase
// și Local/SQLite. NU atinge istoricul: jobs / time_entries / plate_history /
// mileage_log / car_photos rămân neatinse.
function EditCarModal({ car, employees, vehicleMakes = [], vehicleModels = [], workCatalog = [], onClose, onSaved }: {
  car: Car; employees: Employee[]; vehicleMakes?: CatalogOption[]; vehicleModels?: CatalogOption[]; workCatalog?: CatalogOption[];
  onClose: () => void; onSaved: (updated: Car | null) => Promise<void>;
}) {
  const [form, setForm] = useState({
    license_plate: car.license_plate ?? '',
    client_name: car.client_name ?? '',
    client_phone: car.client_phone ?? '',
    ...
    priority: car.priority ?? 'normala',
    deadline: car.deadline ?? '',
    assigned_employee_id: car.assigned_employee_id ?? '',
    is_warranty: Boolean(car.is_warranty),
    notes: car.notes ?? '',
  });
  const selectedMake = vehicleMakes.find((mk) => normalizeSearch(mk.name) === normalizeSearch(form.make));
  const modelOptions = vehicleModels.filter((md) => !form.make || md.make_id === selectedMake?.id);
  catalogFieldOptions = {
    makes: vehicleMakes.length > 0 ? vehicleMakes : VEHICLE_MAKES.map((name) => ({ id: name, name, normalized_name: normalizeSearch(name) })),
    models: modelOptions.length > 0 ? modelOptions : modelsFor(form.make).map((name) => ({ id: name, name, normalized_name: normalizeSearch(name) })),
    works: workCatalog,
  ...
  const update = (key: string, value: string | boolean): void => setForm({ ...form, [key]: value });
  const save = async (): Promise<void> => {
    if (!form.license_plate.trim() || !form.client_name.trim()) { setError('Numărul de înmatriculare și numele clientului sunt obligatorii.'); return; }
    if (!form.mileage.trim()) { setError('Kilometrajul este obligatoriu.'); return; }
    setSaving(true); setError('');
    const nullableStr = (v: string): string | null => (v.trim() ? v.trim() : null);
    const payload: CarUpdateInput = {
      license_plate: form.license_plate.trim().toUpperCase(),
      client_name: form.client_name.trim(),
      client_phone: nullableStr(form.client_phone),
      client_email: nullableStr(form.client_email),
      make: nullableStr(form.make),
      model: nullableStr(form.model),
      color: nullableStr(form.color),
      year: form.year.trim() ? Math.round(Number(form.year)) : null,
      vin: nullableStr(form.vin),
      mileage: Math.round(Number(form.mileage)),
      fuel_level: nullableStr(form.fuel_level) as FuelLevel | null,
      priority: (form.priority || 'normala') as Priority,
      deadline: nullableStr(form.deadline),
      assigned_employee_id: nullableStr(form.assigned_employee_id),
      is_warranty: Boolean(form.is_warranty),
      notes: nullableStr(form.notes),
    };
    if (!dataAdapter.updateCar) { setError('Editarea mașinii nu este disponibilă.'); setSaving(false); return; }
    const { data, error: saveErr } = await dataAdapter.updateCar(car.id, payload);
    if (saveErr) { setError(saveErr.message); setSaving(false); return; }
    setSaving(false);
    await onSaved(data ?? car);
  };
return <Modal title={`Modifică mașina — ${form.license_plate || 'mașină'}`} onClose={onClose} wide><div className="space-y-5 p-6"><div className="grid gap-4 sm:grid-cols-2"><Field label="Număr înmatriculare" value={form.license_plate} onChange={(v: string) => update('license_plate', v)} placeholder="TM 27 FXC" /><Field label="Nume client" value={form.client_name} onChange={(v: string) => update('client_name', v)} placeholder="Ion Popescu" /><Field label="Telefon" value={form.client_phone} onChange={(v: string) => update('client_phone', v)} placeholder="0740 000 000" /><Field label="Email (opțional)" value={form.client_email} onChange={(v: string) => update('client_email', v)} placeholder="client@exemplu.ro" /><Field label="Termen" value={form.deadline} onChange={(v: string) => update('deadline', v)} type="date" /><Field label="An fabricație (opțional)" value={form.year} onChange={(v: string) => update('year', v)} type="number" placeholder="2018" /><Field label="Marcă" value={form.make} onChange={(v: string) => update('make', v)} placeholder="Mercedes" list="servix-makes" /><Field label="Model" value={form.model} onChange={(v: string) => update('model', v)} placeholder="Clasa C" list="servix-models" /><Field label="Culoare (opțional)" value={form.color} onChange={(v: string) => update('color', v)} placeholder="Negru" /><Field label="Serie șasiu / VIN (opțional)" value={form.vin} onChange={(v: string) => update('vin', v)} placeholder="WVWZZZ..." /><Field label="Kilometraj (obligatoriu)" value={form.mileage} onChange={(v: string) => update('mileage', v)} type="number" placeholder="150000" /><label className="text-xs font-bold uppercase tracking-wide text-[var(--text-secondary)]">Nivel carburant (opțional)<select value={form.fuel_level} onChange={(e: React.ChangeEvent<HTMLSelectElement>) => update('fuel_level', e.target.value)} className="mt-2 h-11 w-full rounded-lg border border-[var(--border)] px-3 text-sm font-semibold text-[var(--text-primary)]"><option value="">—</option>{fuelOptions.map((f: FuelLevel) => <option key={f} value={f}>{fuelLabels[f]}</option>)}</select></label><label className="text-xs font-bold uppercase tracking-wide text-[var(--text-secondary)]">Prioritate<select value={form.priority} onChange={(e: React.ChangeEvent<HTMLSelectElement>) => update('priority', e.target.value)} className="mt-2 h-11 w-full rounded-lg border border-[var(--border)] px-3 text-sm font-semibold text-[var(--text-primary)]"><option value="normala">Normală</option><option value="urgenta">Urgentă</option></select></label><label className="text-xs font-bold uppercase tracking-wide text-[var(--text-secondary)]">Responsabil<select value={form.assigned_employee_id} onChange={(e: React.ChangeEvent<HTMLSelectElement>) => update('assigned_employee_id', e.target.value)} className="mt-2 h-11 w-full rounded-lg border border-[var(--border)] px-3 text-sm font-semibold text-[var(--text-primary)]"><option value="">Nealocat</option>{employees.filter((e: Employee) => e.role === 'employee').map((e: Employee) => <option key={e.id} value={e.id}>{e.name}</option>)}</select></label></div><label className="flex items-center gap-2 text-sm font-semibold text-[var(--text-secondary)]"><input type="checkbox" checked={form.is_warranty} onChange={(e: React.ChangeEvent<HTMLInputElement>) => update('is_warranty', e.target.checked)} /> În garanție (tarif 0 lei/oră)</label><label className="block text-xs font-bold uppercase tracking-wide text-[var(--text-secondary)]">Note<textarea value={form.notes} onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => update('notes', e.target.value)} rows={3} className="mt-2 w-full rounded-lg border border-[var(--border)] px-3 py-2 text-sm outline-none focus:border-[var(--primary)]" placeholder="Informații relevante..." /></label><datalist id="servix-makes">{VEHICLE_MAKES.map((m: string) => <option key={m} value={m} />)}</datalist><datalist id="servix-models">{modelsFor(form.make).map((m: string) => <option key={m} value={m} />)}</datalist>{error && <div className="rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm font-semibold text-red-700">{error}</div>}<div className="flex justify-end gap-3 border-t border-[var(--border)] pt-5"><button onClick={onClose} className="rounded-lg px-4 py-2.5 text-sm font-bold text-[var(--text-secondary)] hover:bg-[var(--border)]">Anulează</button><button onClick={save} disabled={saving || !form.license_plate || !form.client_name || !form.mileage} className="rounded-lg px-5 py-2.5 text-sm font-bold text-white transition disabled:bg-[var(--border)] disabled:text-[var(--text-secondary)]" style={{ background: 'var(--button)' }}>{saving ? 'Se salvează...' : 'Salvează modificările'}</button></div></div></Modal>;
}
