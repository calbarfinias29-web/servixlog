...
var SupabaseDataAdapter;
var init_SupabaseDataAdapter = __esm({
  "src/data/SupabaseDataAdapter.ts"() {
    init_supabase();
    SupabaseDataAdapter = class {
      async getEmployees() {
...
var DEFAULT_LOCAL_SERVER_URL, LocalDataAdapter;
var init_LocalDataAdapter = __esm({
  "src/data/LocalDataAdapter.ts"() {
    DEFAULT_LOCAL_SERVER_URL = "http://127.0.0.1:8787";
...
var registry, dataAdapter;
var init_data = __esm({
  "src/data/index.ts"() {
    init_SupabaseDataAdapter();
    init_LocalDataAdapter();
    init_registry();
    init_SupabaseDataAdapter();
    init_LocalDataAdapter();
    init_registry();
    ...
    dataAdapter = new Proxy(registry, {
      get(target, prop) {
...
var APPROVED_PHRASES, NEGATION_WORDS, WRITE_ARMED, ALLOWED_WRITE_ACTIONS, pendingOperationSequence;
var init_agent_security = __esm({
  "src/agent/agent-security.ts"() {
    init_agent_types();
    APPROVED_PHRASES = [
      "da",
...
var ADMIN_WRITE_EXECUTION_ENABLED;
var init_agent_phase3 = __esm({
  "src/agent/agent-phase3.ts"() {
    ADMIN_WRITE_EXECUTION_ENABLED = false;
  }
});
...
var okW, failW, IS_DEMO, EXECUTABLE_KINDS;
var init_agent_write = __esm({
  "src/agent/agent-write.ts"() {
    init_supabase();
    init_data();
    init_agent_phase3();
    okW = (m) => ({ success: true, message: m });
    failW = (m, debugError) => ({ success: false, message: m, debugError });
    IS_DEMO = false;
    EXECUTABLE_KINDS = [
      "update_rates",
      "update_schedule",
...
function cleanupTestRun(testRunId, confirmation) {
  const run = requireRun(testRunId);
  if (!run) return fail2("test_run_id invalid sau inexistent.");
  if (!/^da$/i.test(confirmation.trim())) return fail2("Cleanup necesita confirmarea explicita: Da.");
  ...
  run.finished_at = now();
  return ok2(getCleanupStatus(testRunId));
}
function executeTestManagementTool(tool, params = {}) {
  switch (tool) {
    case "get_test_runs":
      return ok2(getTestRuns());
    case "get_test_run":
      return ok2(getTestRun(String(params.test_run_id ?? "")));
    case "get_test_results":
      return ok2(getTestResults(params.test_run_id ? String(params.test_run_id) : void 0, params.status));
    case "get_bugs":
      return ok2(getBugs(params.test_run_id ? String(params.test_run_id) : void 0, params.severity));
    case "get_bug":
      return ok2(getBug(String(params.bug_id ?? "")));
    case "get_cleanup_status":
      return ok2(getCleanupStatus(String(params.test_run_id ?? "")));
    case "create_test_run":
      return ok2(createTestRun(params));
    case "record_test_result": {
      const result = recordTestResult(params);
      return result ? ok2(result) : fail2("test_run_id invalid sau inexistent.");
    }
    case "create_bug": {
      const bug = createBug(params);
      return bug ? ok2(bug) : fail2("test_run_id invalid sau inexistent.");
    }
    case "update_bug_status": {
      const bug = updateBugStatus(String(params.bug_id ?? ""), params.status);
      return bug ? ok2(bug) : fail2("bug_id invalid sau inexistent.");
    }
    case "cleanup_test_run":
      return cleanupTestRun(String(params.test_run_id ?? ""), String(params.confirmation ?? ""));
    default:
      return fail2(`Tool Faza 6 necunoscut: ${tool}`);
  }
}
...
async function handlePersistentTestManagementMessage(message) {
  ...
  if (pendingPersistentCleanup && /^(nu|anuleaza)$/.test(text)) {
    pendingPersistentCleanup = null;
    return ok2("Cleanup anulat.");
  }
  if (/^(porneste|creeaza) un test run$/.test(text)) return ok2(await createPersistentTestRun({ source: "agent", test_mode: "full-test" }));
  if (/arat(a|a-mi) (ultimul )?test run/.test(text)) return ok2((await getPersistentTestRuns()).slice(0, 1));
  if (/arat(a|a-mi) bug-urile/.test(text)) return ok2(await getPersistentBugs(void 0, /critice/.test(text) ? "CRITICAL" : /high/.test(text) ? "HIGH" : void 0));
  if (/ce teste au esuat|arat(a|a-mi) rezultatul testului/.test(text)) {
    const runId = /\bTR-[\w-]+/.exec(text)?.[0];
    return ok2((await getPersistentTestResults(runId)).filter((result) => !/ce teste au esuat/.test(text) || result.status === "FAIL"));
  }
  const cleanup = /curata test run-ul\s+(TR-[\w-]+)/.exec(text);
  if (cleanup) {
    pendingPersistentCleanup = cleanup[1].toUpperCase();
    return ok2(`Voi \u0219terge DOAR datele asociate test_run_id ${pendingPersistentCleanup}. Datele reale nu vor fi modificate. Confirmi?`);
  }
  if (/status(ul)? cleanup/.test(text)) return fail2("Spune test_run_id pentru statusul cleanup.");
  return null;
}
...
var runs, results, bugs, fixtures, sequence, pendingPersistentCleanup;
var init_agent_test_management = __esm({
  "src/agent/agent-test-management.ts"() {
    init_supabase();
...
var init_agent_full_test = __esm({
  "src/agent/agent-full-test.ts"() {
    init_agent_phase3();
    init_agent_security();
    init_agent_test_management();
  }
});

// src/agent/agent-write-flow.ts
var agent_write_flow_exports = {};
__export(agent_write_flow_exports, {
  cancelPendingOperation: () => cancelPendingOperation,
  confirmPendingOperation: () => confirmPendingOperation,
  convertRomanianNumberWords: () => convertRomanianNumberWords,
  fillScheduleFromCurrent: () => fillScheduleFromCurrent,
  getFlowState: () => getFlowState,
  getPendingOperation: () => getPendingOperation,
  handleAgentMessage: () => handleAgentMessage,
  parseCarWriteIntent: () => parseCarWriteIntent,
  parseRomanianNumberPhrase: () => parseRomanianNumberPhrase,
  parseWriteIntent: () => parseWriteIntent,
  resetFlow: () => resetFlow,
  validateRateValue: () => validateRateValue,
  validateTimeHHMM: () => validateTimeHHMM,
  validateVatValue: () => validateVatValue
});
...
async function confirmPendingOperation(operationId) {
  if (!pending || !pendingPlan || pending.operationId !== operationId) {
    return { success: false, text: "Nu exist\u0103 nicio opera\u021Bie \xEEn a\u0219teptarea confirm\u0103rii." };
  }
...
var flowState, pending, pendingPlan, pendingPreviewLines, RO_UNITS, RO_TENS, RO_NUMBER_WORD, RATE_FIELDS, DAYS, DAY_LABELS, WRITE_VERB, NUMBER, CAR_UPDATE_FIELDS, PLATE_STOPWORDS, CLIENT_ONLY_FIELDS;
var init_agent_write_flow = __esm({
  "src/agent/agent-write-flow.ts"() {
    init_supabase();
    init_data();
    init_agent_security();
    ...
    WRITE_VERB = /(^| )(schimb|schimba|schimbam|modific|modifica|setez|seteaz|pune|pun|actualizez|actualizeaza|readuc|readuce|revin|revina|restabil|inapoi|vreau sa pun|as vrea sa pun|trebuie sa fie din nou)/;
    NUMBER = /(\d+(?:[.,]\d+)?)/;
    CAR_UPDATE_FIELDS = [
    ...
    PLATE_STOPWORDS = /* @__PURE__ */ new Set(["km", "lei", "ani", "vin", "la", "in", "cu", "si"]);
    CLIENT_ONLY_FIELDS = ["client_name", "client_phone", "client_email"];
  }
});

// tests/faza5-agent-integration.test.ts
...
async function get_car(params) {
  try {
    const id2 = params.car_id;
    if (!id2) return fail("car_id obligatoriu");
    const { data, error } = await supabase.from("cars").select("*, jobs(*)").eq("id", id2).single();
    if (error) return fail(error.message);
    return ok(data);
  } catch (err) {
    return fail(err instanceof Error ? err.message : "Eroare");
...
async function get_employee(params) {
  try {
    const id2 = params.employee_id;
    if (!id2) return fail("employee_id obligatoriu");
    const { data, error } = await supabase.from("employees").select("*").eq("id", id2).single();
    if (error) return fail(error.message);
    return ok(data);
  } catch (err) {
    return fail(err instanceof Error ? err.message : "Eroare");
...
async function get_job(params) {
  try {
    const id2 = params.job_id;
    if (!id2) return fail("job_id obligatoriu");
    const { data, error } = await supabase.from("jobs").select("*").eq("id", id2).single();
    if (error) return fail(error.message);
    return ok(data);
  } catch (err) {
    return fail(err instanceof Error ? err.message : "Eroare");
...
async function analyze_productivity() {
  try {
    ...
    const result = employees.map((emp) => {
      ...
      const norm2 = ee.filter((e) => !e.is_overtime).reduce((s, e) => s + (e.duration_seconds ?? 0), 0);
      const ot = ee.filter((e) => e.is_overtime).reduce((s, e) => s + (e.duration_seconds ?? 0), 0);
      return { id: emp.id, name: emp.name, normal_hours: Math.round(norm2 / 3600 * 100) / 100, overtime_hours: Math.round(ot / 3600 * 100) / 100, total_hours: Math.round((norm2 + ot) / 3600 * 100) / 100 };
    });
...
async function executeTool(toolName, params = {}) {
  ...
  if (tool.securityLevel === "write" /* WRITE */ || tool.securityLevel === "delete" /* DELETE */ || tool.securityLevel === "protected" /* PROTECTED */) {
...
function mapIntentToTool(intent) {
  switch (intent.category) {
    case "DASHBOARD":
      return { tool: "get_dashboard", params: {} };
    case "CARS":
      return { tool: "get_cars", params: intent.filters.status ? { status: intent.filters.status } : {} };
    case "JOBS":
      return { tool: "get_jobs", params: intent.filters.status ? { status: intent.filters.status } : {} };
    case "EMPLOYEES":
      return { tool: "get_employees", params: {} };
    case "APPOINTMENTS":
      return { tool: "get_appointments", params: {} };
    case "RATES":
      return { tool: "get_rates", params: {} };
    case "SCHEDULE":
      return { tool: "get_schedule", params: {} };
    case "TIME_ENTRIES":
      return { tool: "get_time_entries", params: {} };
    case "ACTIVITY_LOG":
      return { tool: "get_activity_log", params: {} };
    case "PRODUCTIVITY":
      return { tool: "analyze_productivity", params: {} };
    case "OVERTIME":
      return { tool: "analyze_overtime", params: {} };
    case "COSTS":
      return { tool: "analyze_costs", params: {} };
    case "ANOMALIES":
...
function formatIntentResponse(intent, toolResult) {
...
async function processMessageMulti(message) {
  ...
  const lines = resolved.map((rl, i) => {
    ...
    return resolved.length > 1 ? `${i + 1}. ${CATEGORY_LABELS[rl.intent.category] ?? rl.intent.category}: ${formatted}` : formatted;
  });
  return {
    success: successAll,
    intent: resolved[0].intent,
    toolResult: { success: successAll, data: resolved.map((rl) => rl.intent.category), error: successAll ? void 0 : "A aparut o eroare." },
...
test("READ: detecteaza categoriile naturale", () => {
  ...
  for (const [msg, expected] of cases) {
    assert.equal(detectIntent(msg).category, expected);
  }
...
test("READ: tool-urile de baza sunt READ", () => {
  for (const name of ["get_cars", "get_jobs", "get_employees", "get_rates", "get_schedule"]) {
    const t = AGENT_TOOLS.find((t2) => t2.name === name);
    assert.ok(t);
    assert.equal(t.securityLevel, "read");
  }
...
test("READ: get_dashboard structura valida", async () => {
  ...
  assert.ok(r.success !== void 0);
  if (r.success && typeof r.data === "object" && r.data !== null) {
    assert.ok("cars" in r.data);
    assert.ok("jobs" in r.data);
  }
...
test("READ: get_rates functional", async () => {
  ...
  if (r.success && typeof r.data === "object" && r.data !== null) {
    assert.ok("normal_rate" in r.data || "rates" in r.data);
  }
  pass("READ: get_rates functional");
});
test("READ: get_cars functional", async () => {
  const r = await executeTool("get_cars", {});
  assert.ok(r.success === true || r.success === false);
  pass("READ: get_cars functional");
});
test("READ: get_employees functional", async () => {
  const r = await executeTool("get_employees", {});
  assert.ok(r.success === true || r.success === false);
...
test("READ: analyze_costs functional", async () => {
  ...
  if (r.success && typeof r.data === "object" && r.data !== null) {
    assert.ok("rates" in r.data || "summary" in r.data || "jobs" in r.data);
  }
...
test("WRITE F2: tariful normal", () => {
  ...
  assert.ok(p && !("error" in p));
  assert.equal(p.action, "update_rates");
  assert.deepEqual(p.changes, { normal_rate: 120 });
...
test("WRITE F2: TVA", () => {
  ...
  assert.ok(p && !("error" in p));
  assert.equal(p.action, "update_rates");
  assert.deepEqual(p.changes, { vat_rate: 19 });
...
test("WRITE F2: program sambata", () => {
  ...
  assert.ok(p && !("error" in p));
  assert.equal(p.action, "update_schedule");
  assert.equal(p.changes.saturday_active, true);
  assert.equal(p.changes.saturday_start, "08:00");
  assert.equal(p.changes.saturday_end, "14:00");
...
test("WRITE F2: create_car", () => {
  ...
  assert.ok(p && !("error" in p));
  assert.equal(p.action, "create_car");
  assert.equal(p.changes.license_plate, "B123ABC");
  assert.equal(p.changes.client_name, "Ion Popescu");
...
test("WRITE F2: update_client", () => {
  ...
  assert.ok(p && !("error" in p));
  assert.equal(p.action, "update_client");
  assert.equal(p.changes.client_phone, "0712345678");
...
test("PHASE3: create_employee", () => {
  ...
  assert.ok(p && !("error" in p));
  assert.equal(p.action, "create_employee");
...
test("PHASE3: deactivate_employee", () => {
  ...
  assert.ok(p && !("error" in p));
  assert.equal(p.action, "deactivate_employee");
...
test("PHASE3: transfer_car", () => {
  ...
  assert.ok(p && !("error" in p));
  assert.equal(p.action, "transfer_car");
  assert.ok(p.preview.some((l) => l.includes("RISC: HIGH")));
  assert.ok(p.preview.some((l) => l.includes("TRANSFER") || l.includes("NU SE RESETEAZA")));
...
test("PHASE3: preview contine confirmare", () => {
  ...
  assert.ok(p && !("error" in p));
  assert.ok(p.preview.some((l) => l.includes("CONFIRMARE: DA / NU")));
...
test("PHASE3: delete_employee blocat", () => {
  ...
  assert.ok(p && !("error" in p));
  assert.equal(p.action, "delete_employee");
  assert.equal(ADMIN_WRITE_EXECUTION_ENABLED, false);
...
test("DELETE: cancel_appointment risc HIGH", () => {
  ...
  assert.ok(p && !("error" in p));
  assert.equal(p.action, "cancel_appointment");
  assert.equal(p.risk, "HIGH");
  assert.ok(p.preview.some((l) => l.includes("ATENTIE") || l.includes("DESTRUCTIVA")));
...
test("CONFIRM: Da -> approved", () => {
  assert.equal(parseConfirmation("Da").status, "approved");
  pass("CONFIRM: Da");
});
test("CONFIRM: Nu -> rejected", () => {
  assert.equal(parseConfirmation("Nu").status, "rejected");
  pass("CONFIRM: Nu");
});
test("CONFIRM: Anuleaza -> rejected", () => {
  assert.equal(parseConfirmation("Anuleaza").status, "rejected");
  pass("CONFIRM: Anuleaza");
});
test("CONFIRM: Nu confirm -> rejected", () => {
  assert.equal(parseConfirmation("Nu confirm").status, "rejected");
  pass("CONFIRM: Nu confirm -> rejected");
});
test("CONFIRM: ok -> ambiguous", () => {
  assert.equal(parseConfirmation("ok").status, "ambiguous");
  pass("CONFIRM: ok -> ambiguous");
});
test("CONFIRM: bine -> ambiguous", () => {
  assert.equal(parseConfirmation("bine").status, "ambiguous");
  pass("CONFIRM: bine -> ambiguous");
});
test("CONFIRM: pending ne-permis + Da = blocat", () => {
  const pending2 = createPendingOperation("delete_car", { id: "x" }, {});
  assert.equal(evaluatePendingConfirmation(pending2, parseConfirmation("Da")).canExecute, false);
...
test("CONFIRM: action-not-allowed", () => {
  const pending2 = createPendingOperation("create_employee", { name: "Test" }, { name: "Test" });
  const a = evaluatePendingConfirmation(pending2, parseConfirmation("Da"));
  assert.equal(a.canExecute, false);
  assert.match(a.reason, /action-not-enabled/);
...
test('AMBIGUITY: "Schimba angajatul." -> cere exact', () => {
  ...
  assert.ok(p && "error" in p);
  assert.match(p.error, /Spune exact/);
...
test('AMBIGUITY: "transfera masina la Maria" -> cere numar', () => {
  ...
  assert.ok(p && "error" in p);
  assert.match(p.error, /numarul|numar/i);
...
test('AMBIGUITY: "adauga programare" -> cere data si ora', () => {
  ...
  assert.ok(p && "error" in p);
  assert.match(p.error, /data|necesare/i);
...
test("SECURITY: get_cars este READ", () => {
  const tool = AGENT_TOOLS.find((t) => t.name === "get_cars");
  assert.ok(tool);
  assert.equal(tool.securityLevel, "read");
  pass("SECURITY: get_cars este READ");
});
test("SECURITY: analyze_costs este ANALYZE", () => {
  const tool = AGENT_TOOLS.find((t) => t.name === "analyze_costs");
  assert.ok(tool);
  assert.equal(tool.securityLevel, "analyze");
...
test("SECURITY: allowlist strict 5", () => {
  ...
  for (const a of ALLOWED_WRITE_ACTIONS) {
    assert.equal(isWriteActionAllowed(a), true);
  }
...
test("FULL TEST: test_run_id unic", () => {
  const id1 = createTestRunId();
  const id2 = createTestRunId();
  assert.match(id1, /^test-/);
  assert.match(id2, /^test-/);
  assert.notEqual(id1, id2);
  pass("FULL TEST: test_run_id unic");
});
test("FULL TEST: fixture izolat", () => {
  const fx = createTestFixture2("test-run-123");
...
test("FULL TEST: raport cu cleanup neexecutat", () => {
  ...
  assert.equal(r.testRunId, "test-run-456");
  assert.ok(r.tests.length > 0);
  assert.equal(r.cleanup.executed, false);
  assert.equal(r.cleanup.status, "PENDING_EXPLICIT_COMMAND");
  const f = formatFullTestReport(r);
  assert.match(f, /FULL TEST REPORT/);
  assert.match(f, /TOTAL TESTS:/);
  assert.match(f, /CLEANUP EXECUTED: NO/);
  pass("FULL TEST: raport cu cleanup neexecutat");
});
test("FULL TEST: nu apeleaza fallback", async () => {
  const { handleAgentMessage: handleAgentMessage2, resetFlow: resetFlow2 } = await Promise.resolve().then(() => (init_agent_write_flow(), agent_write_flow_exports));
  resetFlow2();
  let fallbackCalled = false;
  const resp = await handleAgentMessage2("Agent, testeaza programul", async () => {
    fallbackCalled = true;
    return { success: true, text: "unexpected" };
  });
  assert.equal(resp.success, true);
  assert.equal(fallbackCalled, false);
  assert.match(resp.text, /FULL TEST REPORT/);
...
test("ORCHESTRATOR: handleAgentMessage -> fallback READ", async () => {
  const { handleAgentMessage: handleAgentMessage2, resetFlow: resetFlow2 } = await Promise.resolve().then(() => (init_agent_write_flow(), agent_write_flow_exports));
  resetFlow2();
  const resp = await handleAgentMessage2("Cati angajati avem?", async () => {
    const r = await processMessageMulti("Cati angajati avem?");
    return { success: r.success, text: r.formattedResponse };
  });
...
test("ORCHESTRATOR: WRITE nou inlocuieste pending vechi", async () => {
  const { handleAgentMessage: handleAgentMessage2, resetFlow: resetFlow2, getPendingOperation: getPendingOperation2 } = await Promise.resolve().then(() => (init_agent_write_flow(), agent_write_flow_exports));
  resetFlow2();
  await handleAgentMessage2("Schimba tariful normal la 100", async () => ({ success: true, text: "" }));
  const first = getPendingOperation2();
  assert.ok(first);
  await handleAgentMessage2("Schimba TVA la 19%", async () => ({ success: true, text: "" }));
  const second = getPendingOperation2();
...
test('ORCHESTRATOR: "Da" fara pending -> nu executa', async () => {
  const { handleAgentMessage: handleAgentMessage2, resetFlow: resetFlow2 } = await Promise.resolve().then(() => (init_agent_write_flow(), agent_write_flow_exports));
  resetFlow2();
  const resp = await handleAgentMessage2("Da", async () => ({ success: true, text: "fallback" }));
...
if (failed > 0) {
  console.error("\nFAZA 5: FAIL - " + failed + " teste esuate");
  process.exit(1);
} else {
