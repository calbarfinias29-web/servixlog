...
function normalizeConfirmationText(text) {
  return String(text ?? "").toLowerCase().normalize("NFC").replace(/[ş]/g, "s").replace(/[ţ]/g, "t").replace(/ș/g, "s").replace(/ț/g, "t").replace(/ă/g, "a").replace(/â/g, "a").replace(/î/g, "i").replace(/[^a-z0-9 ]/g, " ").replace(/\s+/g, " ").trim();
}
...
test("WRITE ramane DEZACTIVAT (WRITE_ARMED === false)", () => {
  assert.equal(WRITE_ARMED, false);
});
...
test('SECURITY: "Da" fara pending confirmation NU poate executa nimic', () => {
  ...
  assert.equal(approval.canExecute, false);
  assert.equal(approval.reason, "write-disabled (FAZA 2A)");
});
