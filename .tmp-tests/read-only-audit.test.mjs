...
// tests/read-only-audit.test.ts
var auditMessage = `Vreau s\u0103 faci un AUDIT COMPLET AL APLICA\u021AIEI SERVIX.
NU modifica nimic.
NU crea nimic.
NU \u0219terge nimic.
NU schimba tarife.
...
async function readOnlyResponse(message) {
  resetFlow();
  let fallbackCalls = 0;
  const response = await handleAgentMessage(message, async () => {
    fallbackCalls += 1;
    return { success: true, text: "ANALYZE ONLY" };
  });
...
test("verificarea programului si auditul masinilor raman ANALYZE", async () => {
  await readOnlyResponse("Verific\u0103 dac\u0103 programul este configurat corect. Nu \xEEl modifica.");
