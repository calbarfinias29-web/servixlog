// tests/write-intent-live-bug.test.ts
...
test("explicit rate WRITE creates preview and pending without executing", async () => {
  ...
  const pending2 = getPendingOperation();
  assert.equal(fallbackCalled, false);
  assert.equal(response.success, true);
  assert.equal(response.pending?.action, "update_rates");
  assert.match(response.text, /101/);
  assert.ok(pending2?.operationId);
  assert.equal(pending2?.proposedChanges?.normal_rate, 101);
});
test("read-only tariff hypothetical does not create pending WRITE", async () => {
  resetFlow();
  let fallbackCalled = false;
  const response = await handleAgentMessage("Verific\u0103 dac\u0103 tariful normal este 100 lei/or\u0103 \u0219i analizeaz\u0103 ce s-ar \xEEnt\xE2mpla dac\u0103 ar fi 101 lei/or\u0103.", async () => {
    fallbackCalled = true;
...
test("restore wording remains explicit update_rates WRITE", () => {
  const cases = [
    ...
    "Pune tariful normal \xEEnapoi la 100 lei/or\u0103.",
    "Schimb\u0103 tariful normal \xEEnapoi la 100 lei/or\u0103.",
...
NU executa \xEEnc\u0103.`;
  ...
  assert.equal(fallbackCalled, false);
  assert.equal(response.pending?.action, "update_rates");
  assert.equal(pending2?.proposedChanges?.normal_rate, 100);
  assert.ok(pending2?.operationId);
